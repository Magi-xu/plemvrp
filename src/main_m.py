...
def run(data, progress_dict):
    ...
    for n_iter in range(ITERATIONS):
        ...
        progress_dict[data] = f"迭代次数: {n_iter + 1}/{ITERATIONS} 车辆数量：{best_solution.vehicleNumber} 距离：{format(best_solution.distance, '.4f')}"
        ...
        if (n_iter + 1) % 1 == 0:
            ...
            with open("../results/solomon_100/" + data + "/best_solution.txt", "w") as f:
                f.write(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + "\t" + data + "\t" + "iter: " + str(n_iter + 1) + "\n")
                f.write("vehicleNumber=" + str(best_solution.vehicleNumber) + "\t" + "distance=" + str(best_solution.distance) + "\n")
    ...
    #             if p.id != route.customers[-1].id:
    #                 f.write(" --> ")
    #         f.write("waitTime=" + route.waitTime + "\n")
    # print("\t完成")
    progress_dict[data] = "完成"
...
if __name__ == '__main__':
    ...
    for benchmark in benchmarks:
        pool.apply_async(run, args=(benchmark, progress_dict))

    # 关闭进程池
    pool.close()

    # 在等待进程完成的同时，定期报告所有进程的进度
    try:
        while True:
            time.sleep(5)  # 每秒按benchmarks的顺序报告一次进度
            info = [[benchmark, progress] for benchmark, progress in progress_dict.items()]
            # 按benchmarks的顺序报告进度
            for benchmark in benchmarks:
                for i in range(len(info)):
                    if info[i][0] == benchmark:
                        print(f"测例 {benchmark}: {info[i][1]}")
                        break
