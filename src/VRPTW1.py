...
def readCSVFile(filename, model):
    data = {}
    with open(filename, 'r') as file:
        ...
        # Read depot data
        line = lines[9]
        depot_data = line.split()
        node = Node()
        node.id = int(depot_data[0])
        node.x_coord = float(depot_data[1])
        node.y_coord = float(depot_data[2])
        node.depot_capacity = float(data["vehicle_capacity"])
        node.start_time = float(depot_data[4])
        node.end_time = float(depot_data[5])
        ...
        for line in lines[10:]:
            ...
            node = Node()
            node.id = int(customer_data[0])
            node.x_coord = float(customer_data[1])
            node.y_coord = float(customer_data[2])
            node.demand = float(customer_data[3])
            node.start_time = float(customer_data[4])
            node.end_time = float(customer_data[5])
            node.service_time = float(customer_data[6])
...
def run(filename, epochs, pc, pm, popsize, n_select, v_cap, v_speed, opt_type):
    model = Model()
    ...
    for ep in range(epochs):
        ...
        print("%s/%s， best obj: %s,  route num: %s" % (ep, epochs, model.best_sol.obj, len(model.best_sol.route_list)))
...
if __name__ == '__main__':
    filename = '../data/solomon_100/c101.txt'
    run(filename=filename, epochs=100, pc=0.8, pm=0.1, popsize=100,
